...
exports.createRental = async (req, res) => {
    ...
    const parsedItems = JSON.parse(items); // Karena dikirim sebagai FormData, perlu di-parse
    ...
    try {
        ...
        for (const item of parsedItems) {
            ...
            if (deliveryOption === 'delivery') {
                totalPrice += 25000; // Contoh biaya pengiriman Rp 25.000
            }
...
exports.updateRentalStatus = async (req, res) => {
    ...
    try {
        ...
        // 1. Update status di tabel rentals
        await connection.execute(
            'UPDATE rentals SET rental_status = ? WHERE id = ?',
            [status, id]
        );

        // 2. Jika dibatalkan atau selesai, ubah status laptop kembali menjadi 'available'
